...
class Detector:
    ...
    def onImage(self, imagePath, buffer_size = 8):
        # Load the image
        ...
        for i in tqdm(range(len(instances)), desc="Processing instance masks"):
            ...
            mask_dilated = cv2.dilate(mask.numpy().astype(np.uint8), kernel, iterations=buffer_size)
            mask_image = np.maximum(mask_image, mask_dilated)
